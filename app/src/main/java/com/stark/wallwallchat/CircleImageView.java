...
public class CircleImageView extends ImageButton {
    ...
    public void setHead(Context context,String id) {
        if (id == null) {
            CircleImageView.this.setImageDrawable(getResources().getDrawable(R.drawable.default_head));
            return;
        }
        mContext=context;
        receiver=id;
        db=new DatabaseHelper(context).getWritableDatabase();
        Cursor cursor = db.query("userdata",new String []{"head"}, "id=?", new String[]{id}, null, null, null);
        if(cursor!=null&&cursor.getCount()>0&&cursor.moveToNext()) {
            if(cursor.getString(cursor.getColumnIndex("head"))!=null&&!cursor.getString(cursor.getColumnIndex("head")).equals("")){
                Cursor cr = db.query("file",null, "hashcode=?", new String[]{cursor.getString(cursor.getColumnIndex("head"))}, null, null, null);
                if(cr!=null&&cr.getCount()>0&&cr.moveToNext()) {
                    String path=cr.getString(cr.getColumnIndex("path"));
                    File file=new File(path);
                    if(file.exists()){
                        CircleImageView.this.setImageDrawable(new BitmapDrawable(BitmapFactory.decodeFile(path)));
                    }else{
                        db.delete("file","hashcode=?",new String[]{cr.getString(cr.getColumnIndex("hashcode"))});
                        hashcode=cr.getString(cr.getColumnIndex("hashcode"));
                        new Thread(GetPic).start();
                    }
                    cr.close();
                }else{
                    hashcode=cursor.getString(cursor.getColumnIndex("head"));
                    new Thread(GetPic).start();
                }
            }else{
    ...
    Handler handle=new Handler(new Handler.Callback() {
        ...
        public boolean handleMessage(Message msg) {
            switch (msg.what){
                case 0:
                    String path=FileUtil.getPath(FileType.Head)+"/"+hashcode+".png";
                    Log.e("path", path);
                    File file=new File(path);
                    if(file.exists()){
                        ContentValues cv=new ContentValues();
                        cv.put("hashcode",hashcode);
                        cv.put("name",hashcode+".png");
                        cv.put("path",path);
                        cv.put("time",new SimpleDateFormat("yyyyMMddHHmmssSSS", Locale.CHINA).format(new Date()));
                        db.insert("file",null,cv);
                        CircleImageView.this.setImageDrawable(new BitmapDrawable(BitmapFactory.decodeFile(path)));
                    }else{
                        CircleImageView.this.setImageDrawable(getResources().getDrawable(R.drawable.default_head));
                    }
                    if(mContext.getSharedPreferences("action",Context.MODE_PRIVATE).getString("id","").equals(receiver)){
                        Intent intent=new Intent();
                        intent.setAction("com.stark.wallwallwallchat.changeHead");
    ...
    Runnable GetUser=new Runnable() {
        ...
        public void run() {
            ...
            SqlPkg.put("receiver",receiver);
            NetPackage netPkg=new NetPackage(mContext,SqlPkg);
            try{
                String temp=JsonConvert.SerializeObject(netPkg.UserInfo(1));
                NetBuilder out=(NetBuilder)JsonConvert.DeserializeObject(NetSocket.request(temp), new NetBuilder());
                if(out.getBool("flag",false)) {
                    out.UpdateDB(db, mContext.getSharedPreferences("action", Context.MODE_PRIVATE));
                    if(out.get("head")!=null) {
                        hashcode=out.get("head");
                        handle.obtainMessage(100).sendToTarget();
                    }else{
                        handle.obtainMessage(13).sendToTarget();
                    }
                }else{
                    handle.obtainMessage(out.getInt("error",13)).sendToTarget();
                }
            }catch (Exception e){
                handle.obtainMessage(13).sendToTarget();
            }
    ...
    Runnable GetPic=new Runnable() {
        ...
        public void run() {
            HashMap<String,Object> SqlPkg=new HashMap<String,Object>();
            SqlPkg.put("hashcode",hashcode);
            SqlPkg.put("thumb", true);
            SqlPkg.put("width", 200);
            SqlPkg.put("height",200);
            SqlPkg.put("mode", "HW");
            NetPackage netPkg=new NetPackage(SqlPkg);
            try {
                String temp = JsonConvert.SerializeObject(netPkg.DownFile());
                NetSocket.request(temp, FileUtil.getPath(FileType.Head),new UpFileListener() {
                    @Override
                    ...
                    public void error(int error) {
                        handle.obtainMessage(error).sendToTarget();
                    }
                    @Override
